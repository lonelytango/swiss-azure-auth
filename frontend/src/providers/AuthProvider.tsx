...
import { AuthContext, type AuthContextValue, type UserData } from './auth-context'
...
function AuthSessionProvider({ children }: { children: React.ReactNode }) {
  ...
  const accessToken = useMemo(() => {
    if (!activeAccount || !isAuthenticated || inProgress !== InteractionStatus.None) {
      return null
    }
  ...
  const user = useMemo<UserData>(() => {
    ...
    return {
      ...
      lastName,
      email: (activeAccount.idTokenClaims?.email as string) || activeAccount.username || '',
  ...
  useEffect(() => {
    const initMsal = async () => {
      try {
        ...
        setInitError(err instanceof Error ? err.message : 'MSAL initialization failed.')
      } finally {
        setIsReady(true)
      }
    }
